...
<?php 
    ...
    $stmnt = $conn->prepare("SELECT user_id FROM `tbl_user` WHERE user_id NOT IN (select user_id from tbl_access where group_id=0 and chat_header_id=:chat_header_id)");
    $stmnt->bindParam("chat_header_id",$chat_header_id);
    $stmnt->execute();
    $records = $stmnt->fetchAll();
    ...
    if($records==NULL && empty($_GET['check_group']))
    	echo "<script>window.location.href='UpdateMessage.php?chat_header_id=$chat_header_id'</script>";

    foreach ($records as $key => $value) {
    	# code...
    	$all_users_array[] = $value['user_id'];
    }
        ...
        <div class="col-sm-6">
            <form id="myform" method="POST" action="" data-toggle="validator" role="form">
              ...
              <input type="hidden" name="message_type" value="<?php echo $message_type; ?>"> 
               ...
               <?php 
               if(!empty($_GET['all_users']) && !empty($all_users_array))
               {
               ...
               echo '
                ...
                <table class="table table-striped table-bordered" id="mytable">
                    ...
                    foreach ($records as $record) {
                        ...
                        if(in_array($each_user_id, $all_users_array))
                        echo "
                            <tr>
	                            <td>$name</td>
                    ...
                    if(!empty($_GET['check_group']))
                     {
                     	$group_string = $_GET['check_group'];

                     	$check_group = explode(",", $group_string);
                    ...
                    foreach ($groups as $group) {
					    ...
					    foreach ($members as $member) {
                            ...
                            echo "
                                <tr>
                                    <td>$member_name</td>
                                    <td>Read & write<input type='hidden' name='perm_write[$each_group_id][]' value='$member_user_id'></td>                  
                            ...
                            echo "
                                <tr>
                                    <td>$member_name</td>
                                    <td>Readonly<input type='hidden' name='perm_read[$each_group_id][]' value='$member_user_id'></td>                  
